...
export class ReferenceFearUseCase {
  ...
  async execute(
    ...
    let tags: ITag[]
    tags = project.tags.filter((tag) => tag.type !== 'persons/fears')
    const tagFears = project.tags.find(
      (tag) => tag.type === 'persons/fears',
    ) as ITag
    ...
    const exiteRef = tagFears.refs.find((ref) => ref.object.id === refId)
    ...
    const filteredRefs = tagFears.refs.filter((ref) => ref.object.id !== refId)
