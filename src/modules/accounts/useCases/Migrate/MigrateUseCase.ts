...
export class MigrateUseCase {
  ...
  async execute(executeTime: string): Promise<void> {
    ...
    switch (executeTime) {
      case '1': {
        ...
        const lengthArrayResults = Math.ceil(arrayLength / 4)

        const personsToCreate = persons.slice(3 * lengthArrayResults)

        await Promise.all(
          personsToCreate.map(async (p) => {
            const pm = p as unknown as IPersonMongo
      ...
      case '3': {
        ...
        boxesNotInternal.map((b) =>
          b.archives.map((a) =>
            archivesToCreate.push({ boxId: b.id, archive: a }),
          ),
        )
